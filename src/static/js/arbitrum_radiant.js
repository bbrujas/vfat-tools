...
async function main() {
  ...
  await loadRDNTChefContract(App, tokens, prices, RDNT_CHEF, RDNT_CHEF_ADDR, RDNT_CHEF_ABI, rewardTokenTicker,
    "0x3082CC23568eA640225c2467653dB90e9250AaA0", null, rewardsPerWeek);

  _print("");
  _print_bold("V2 Contracts");
  _print("");

  const RDNT_CONTRACT_ADDR_V2 = "0xF4B1486DD74D07706052A33d31d7c0AAFD0659E1";
  const RDNT_CONTRACT_V2 = new ethers.Contract(RDNT_CONTRACT_ADDR_V2, RDNT_CONTRACT_ABI, App.provider);

  const reservesList_v2 = await RDNT_CONTRACT_V2.getReservesList();

  await Promise.all(reservesList_v2.map(a => loadRDNTDetails(App, a, A_TOKEN_ABI, B_TOKEN_ABI, RDNT_CONTRACT_V2, prices)));

  _print("");
  _print_bold("V1 Contracts");
  _print("");
